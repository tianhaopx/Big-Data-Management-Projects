...
db.test.aggregate([{$unwind:"$contribs"},{$match:{contribs:{$in:array}}},{$group:{_id:"$contribs",people:{$addToSet:"$name"}}}])
...
db.test.aggregate([{$unwind:"$awards"},{$match:{"awards.year":2001}},{$group:{_id:"$name",num:{$sum:1}}},{$match:{num:{$gte:2}}},{$project:{num:0}}])
